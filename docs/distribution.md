...
The current production release channel is the public npm registry. Versioning is source-controlled with Changesets,
...
This package publishes to npm as `@launcheleven/srgical`.

The workflow publishes with the repository secret `NPM_TOKEN`. Local publishes require the same token in the user's
own npm config:
